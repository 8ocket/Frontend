...
export interface ChatMainAreaProps {
  ...
  readonly onEndChat?: () => void;
  /** 크레딧 부족 시 호출 */
  readonly onCreditShortage?: () => void;
  /** 미완결 상담 발견 시 호출 */
  readonly onUnfinishedSession?: () => void;
  /** 표시할 메시지 목록 */
  readonly initialMessages?: ChatBubbleProps[];
  /** 활성 세션 여부 — false면 입력창 비활성화 */
  readonly isSessionActive?: boolean;
  /** 비활성 입력창/전송 버튼 클릭 시 호출 */
  readonly onDisabledInputClick?: () => void;
  /** 외부에서 채팅에 추가할 메시지 — 변경될 때마다 목록에 append */
  readonly appendMessage?: ChatBubbleProps | null;
  ...
  /** AI 상담사 아바타 이미지 URL */
  readonly aiAvatarSrc?: string;
  /** 사용자가 메시지를 전송할 때 호출 — 60분 자동 종료 타이머 리셋용 */
  readonly onUserMessage?: () => void;
  /** 사용자 닉네임 */
...
export function ChatMainArea({
  ...
  // initialMessages 최신값 보장을 위해 ref 동기화 방식으로 수정 : nelee
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (sessionId === undefined) {
      ...
      // effect 내부 동기 setState 경고 회피를 위해 queueMicrotask 방식으로 수정 : nelee
      queueMicrotask(() => {
        setMessages([]);
