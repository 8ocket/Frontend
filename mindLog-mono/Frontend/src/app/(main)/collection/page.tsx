...
import { EMOTION_META } from '@/widgets/emotion-card';
...
/** SummaryResponse.emotion 설명 텍스트에서 한글 감정어를 찾아 영문 라벨 반환 */
function deriveEmotionLabel(emotionText: string | null): string | null {
  if (!emotionText) return null;
  for (const [, meta] of Object.entries(EMOTION_META)) {
    if (emotionText.includes(meta.label)) {
      return meta.intensityLabels.intense.toUpperCase();
    }
  }
  return null;
}
...
function CardOverlay({ data, onClose }: { readonly data: SummaryListItem; readonly onClose: () => void }) {
  const W = 400;
  const H = 686;
  const [detail, setDetail] = useState<SummaryResponse | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const emotionLabel = deriveEmotionLabel(detail?.emotion ?? null);
          ...
          >
            ...
            >
              <div className="relative h-full w-full">
                ...
                {emotionLabel && (
                  <>
                    <span className="card-01 absolute top-4 left-4 z-10 select-none pointer-events-none text-prime-900">
                      {emotionLabel}
                    </span>
                    <span className="card-01 absolute bottom-4 right-4 z-10 rotate-180 select-none pointer-events-none text-prime-900" aria-hidden>
                      {emotionLabel}
                    </span>
                  </>
                )}
