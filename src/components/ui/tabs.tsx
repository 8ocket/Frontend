...
function TabsList({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.List>) {
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={cn(
        'border-cta-300 dark:border-cta-400 inline-flex h-11 items-center justify-center gap-4 rounded-md border px-1',
        className
      )}
      {...props}
    />
...
function TabsTrigger({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.Trigger>) {
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        'text-prime-500 dark:text-prime-400 inline-flex h-9 w-35 items-center justify-center rounded-md p-2.5',
