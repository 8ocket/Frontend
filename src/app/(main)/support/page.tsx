  ...
  return (
    <div className="mx-auto max-w-7xl px-10 py-10">
      ...
      <div className="mx-auto max-w-4xl">
        <p className="text-prime-800 mb-6 text-[20px] leading-tight font-semibold">
          자주 묻는 질문입니다.
          <br />
          문의를 주시기 전에 먼저 읽어주시면 감사드리겠습니다.
        </p>

        <ul className="divide-prime-100 divide-y">
          {FAQ_ITEMS.map((item) => (
            <li key={item.id}>
              <button
                type="button"
              ...
              >
                <span className="body-1 text-prime-900 font-medium">{item.question}</span>
                <ChevronToggleIcon
                  open={openIds.has(item.id)}
                  className="text-prime-900 ml-4 shrink-0"
                  size={13}
                />
              </button>
              ...
              {openIds.has(item.id) && (
                <div className="pr-10 pb-4">
                  <p className="body-2 text-prime-700">{item.answer || '답변을 준비 중입니다.'}</p>
                </div>
              )}
            </li>
          ))}
